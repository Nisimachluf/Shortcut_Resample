...
class TimestepEmbedder(nn.Module):
    ...
    def __call__(self, t):
        ...
        x1 = nn.Dense(self.hidden_size, kernel_init=nn.initializers.normal(0.02), 
                     bias_init=self.tc.kern_init('time_bias'), dtype=self.tc.dtype)(x_e[0])
        x = nn.silu(x1)
    ...
    def timestep_embedding(self, t, max_period=10000):
        ...
        embedding1 = jnp.concatenate([jnp.cos(args), jnp.sin(args)], axis=-1)
        embedding = embedding1.astype(self.tc.dtype)
...
class MlpBlock(nn.Module):
    ...
    def __call__(self, inputs):
        ...
        # print("actual_out_dim", actual_out_dim)
...
class DiTBlock(nn.Module):
    ...
    def __call__(self, x, c):
        ...
        return x, x_norm, x_modulated, shift_msa, scale_msa, c.copy(), cact
...
class DiT(nn.Module):
    ...
    def __call__(self, x, t, dt, y, train=False, return_activations=False):
        ...
        te, *_ = TimestepEmbedder(self.hidden_size, tc=tc)(t) # (B, hidden_size)
        dte, *_ = TimestepEmbedder(self.hidden_size, tc=tc)(dt) # (B, hidden_size)
        ...
        for i in range(self.depth):
            x, *_ = DiTBlock(self.hidden_size, self.num_heads, tc, self.mlp_ratio, self.dropout, train)(x, c)
            interms.append(x.copy())
        ...
        return x #, pos_embed, te, dte, ye, *interms, true_input_x, x_patch_embed, xpos_embed
