...
flags.DEFINE_integer('inference_generations', 2, 'Number of generations for inference.')
...
def do_inference(
    ...
    with jax.spmd_mode('allow_all'):
        ...
        if FLAGS.model.use_stable_vae:
            ...
            print("New batch shape:", batch_images.shape)
        ...
        print(num_generations, FLAGS.batch_size, num_generations // FLAGS.batch_size)
        for fid_it in tqdm.tqdm(range(num_generations // FLAGS.batch_size)):
            ...
            for ti in tqdm.tqdm(range(denoise_timesteps)):
        ...
        if jax.process_index() == 0:
            ...
            if FLAGS.save_dir is not None:
                ...
                np.save(FLAGS.save_dir + f'/x_render.npy', x_render)
                x_render = x_render.clip(-1, 1)
                x_render = x_render *0.5 + 0.5
                x_render = (x_render*255).astype(np.uint8)
                for i, img in enumerate(x_render):
                    plt.imsave(os.path.join(FLAGS.save_dir, f"res_{i}.png"), img)
