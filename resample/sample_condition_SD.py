...
for i, ref_img in enumerate(loader):
    ...
    psnr_label = psnr(true, label)
    psnr_cur = psnr(true, reconstructed)

    # Saving images
    plt.imsave(os.path.join(out_path, 'input', fname+'_true.png'), true)
    plt.imsave(os.path.join(out_path, 'label', fname+f'_label_{psnr_label:.3f}.png'), label)
    plt.imsave(os.path.join(out_path, 'recon', fname+f'_recon_{psnr_cur:.3f}.png'), reconstructed)
